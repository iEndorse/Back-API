...
async function createVideoFromImages(imagePaths, outputVideoPath, duration = 50) {
    return new Promise((resolve, reject) => {
        ...
        // Calculate duration per image to fit the total duration
        // Use Math.max to avoid division by zero if duration is very small
        const durationPerImage = Math.max(0.1, duration / totalImages); // Ensure at least 0.1s per image
        const outputFps = 25; // Standard video frame rate

        console.log(`Creating video from ${totalImages} images. Duration per image: ${durationPerImage.toFixed(2)}s. Total duration: ${duration}s.`);
        ...
        imagePaths.forEach(imgPath => {
            // FFmpeg requires paths with forward slashes, even on Windows, in the list file
            const ffmpegPath = imgPath.replace(/\\/g, '/');
            fileContent += `file '${ffmpegPath}'\n`;
            fileContent += `duration ${durationPerImage}\n`;
        });
         // Add the last image again to ensure the duration calculation is correct for concat
        ...
        command
            .input(listFilePath)
            .inputOptions(['-f concat', '-safe 0']) // Use concat demuxer, allow unsafe paths (needed for relative/absolute paths)
            .outputOptions([
                '-vsync vfr',                 // Variable frame rate sync to handle image durations
                `-t ${duration}`,             // Set total output duration
