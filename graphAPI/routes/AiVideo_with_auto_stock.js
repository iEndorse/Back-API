...
// Build intelligent search query from campaign context
function buildPexelsSearchQuery({ category, intent, campaignTitle, campaignDescription, segmentText }) {
  ...
  // Combine and deduplicate keywords, prioritizing title > description > segment
  const allKeywords = [
  ...
  // Intent-specific visual modifiers
  const intentVisuals = {
  ...
  // Build query: top keywords + one intent modifier
  let query = '';
  
  if (uniqueKeywords.length > 0) {
    // Use top 2-3 specific keywords
    query = uniqueKeywords.slice(0, 3).join(' ');
    
    // Add one intent modifier for visual variety
    query += ` ${intentTerms[0]}`;
  } else {
    // Fallback to category if no keywords extracted
...
// Fetch video from Pexels API with intelligent search
async function fetchPexelsVideo({ apiKey, category, intent, campaignTitle, campaignDescription, segmentText }) {
  ...
  try {
    const response = await axios.get('https://api.pexels.com/videos/search', {
      params: {
        query,
        orientation: 'portrait', // 9:16 vertical videos
        size: 'medium',
        per_page: 15
      },
      headers: {
    ...
    if (!videos.length) {
      // Fallback: try with just the top keyword
      const keywords = extractKeywords(campaignTitle || campaignDescription || '');
...
// Download Pexels video to local temp file
async function downloadPexelsVideo({ apiKey, category, intent, campaignTitle, campaignDescription, segmentText }) {
  ...
  // Get HD video file (portrait/vertical format)
...
async function fillBackgroundWithPexels({ plan, category, pexelsApiKey, campaignTitle, campaignDescription, segments }) {
  for (let i = 0; i < plan.length; i++) {
    ...
    try {
      ...
      // Continue without background - will error later if all segments fail
    }
  }
  return plan;
}
