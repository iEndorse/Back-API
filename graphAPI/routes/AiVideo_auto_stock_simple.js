...
/* ============================
   🎬 PEXELS VIDEO SEARCH - SIMPLIFIED
   No intent classification, just topic-focused search
============================ */
...
// Simplified: Build query based ONLY on script topic (no intent modifiers)
function buildPexelsSearchQuery({ category, scriptTitle, scriptDescription }) {
  // PRIORITY 1: Use generated script title/description
  const titleKeywords = extractKeywords(scriptTitle);
  const descKeywords = extractKeywords(scriptDescription);

  // Build core topic keywords
  const coreTopicKeywords = [
    ...titleKeywords.slice(0, 2),
    ...descKeywords.slice(0, 1)
  ];

  const uniqueCoreKeywords = [...new Set(coreTopicKeywords)];

  let query = '';
  
  if (uniqueCoreKeywords.length > 0) {
    // Use top 2-3 keywords from script
    query = uniqueCoreKeywords.slice(0, 3).join(' ');
  } else {
    // Fallback to category
    const cat = normalizeCategory(category);
    query = cat.replace(/_/g, ' ');
  }

  return query.trim();
}

// Fetch random video from Pexels based on topic
async function fetchPexelsVideo({ apiKey, category, scriptTitle, scriptDescription }) {
  ...
  try {
    const response = await axios.get('https://api.pexels.com/videos/search', {
      params: {
        ...
        size: 'medium',
        per_page: 30  // Get more results for better randomization
      },
      headers: {
    ...
    if (!videos.length) {
      // Fallback: use top keyword only
      const keywords = extractKeywords(scriptTitle || scriptDescription || '');
      ...
      // Return random video from fallback results
      return fallbackVideos[Math.floor(Math.random() * fallbackVideos.length)];
    }

    // Return random video from results (ensures variety on each request)
...
async function downloadPexelsVideo({ apiKey, category, scriptTitle, scriptDescription }) {
  ...
  console.log(`[Pexels] Downloading video from: ${video.user?.name || 'Pexels'}`);
...
/* ============================
   Render plan with Pexels - Simplified
   Same video for all segments, randomly selected
============================ */
...
// Get DIFFERENT random Pexels videos for each segment
async function fillBackgroundWithPexels({ plan, category, pexelsApiKey, scriptTitle, scriptDescription }) {
  for (let i = 0; i < plan.length; i++) {
    const item = plan[i];
    if (item.backgroundVideo) continue;
    
    try {
      // Download a DIFFERENT random video for each segment
      ...
      console.log(`[Pexels] Segment ${i + 1}: ${path.basename(item.backgroundVideo)}`);
    } catch (err) {
      console.error(`Failed to fetch Pexels video for segment ${i + 1}:`, err.message);
      // Continue - will error later if all segments fail
    }
  }
